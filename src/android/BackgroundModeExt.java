...
public class BackgroundModeExt extends CordovaPlugin {
    ...
    private void openAppStart (Object arg)
    {
		...
		final Intent intentFinal = intent;
                ...
                dialog.setPositiveButton(ok, new DialogInterface.OnClickListener() { public void onClick(DialogInterface dialog, int which) {activity.startActivity(intentFinal);}});
                dialog.setNegativeButton(cancel, new DialogInterface.OnClickListener() { public void onClick(DialogInterface dialog, int which) {}});
     ...
     public void run() {dialogFinal.show();}});
    ...
    {
        getApp().runOnUiThread(new Runnable() {
     public void run() {getApp().getWindow().addFlags(FLAG_ALLOW_LOCK_WHILE_SCREEN_ON | FLAG_SHOW_WHEN_LOCKED | FLAG_TURN_SCREEN_ON | FLAG_DISMISS_KEYGUARD);}});
    }
    ...
    {
        getApp().runOnUiThread(new Runnable() {
     public void run() {getApp().getWindow().clearFlags(FLAG_ALLOW_LOCK_WHILE_SCREEN_ON | FLAG_SHOW_WHEN_LOCKED | FLAG_TURN_SCREEN_ON | FLAG_DISMISS_KEYGUARD);}});
    }
	...
	final Activity appFinal = app;
     ...
     public void run() {appFinal.getWindow().clearFlags(FLAG_DISMISS_KEYGUARD);}});
    }
